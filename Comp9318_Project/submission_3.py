...
def fool_classifier(test_data):  ## Please do not change the function defination...
    ...
    traverse_list.sort(reverse=False)
    ...
    for e in range(len(test_set)):
        for el in traverse_list:
            ...
            for els in test_set[e]:
                _sample = set(test_set[e])
                if len((set(record) - set(_sample)) | (set(_sample) - set(record))) >= 20: break
                newone = els.rstrip(',').rstrip(';').rstrip('.')
                if dict_x_train[_index] == newone:
                    test_set[e].remove(els)
