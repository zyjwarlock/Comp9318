        ...
        while len((set(record) - set(set(test_set[e]))) | (set(set(test_set[e])) - set(record)))<20:
            ...
            for e_n in range(en, len(feature_list_nag)):
                if (feature_list_nag[e_n][0] not in test_set[e]):
                    ...
                    break
            if(len(word_del)!=0): test_set[e] = list(filter(lambda x:x!=word_del[0], test_set[e]))
            if(len((set(record) - set(set(test_set[e]))) | (set(set(test_set[e])) - set(record)))>=20): break
            if (len(word_add) != 0): test_set[e].append(word_add[0])
            ep = e_p
            en = e_n
