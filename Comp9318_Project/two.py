...
def fool_classifier(test_data):  ## Please do not change the function defination...
    ...
    while flag:
        ...
        flag = False
        if list_count[0] != 20:
            if cur>0 and dict_x_train[_index] in test_set[0]:
                test_set[0].remove(dict_x_train[_index])
                list_count[0] += 1
            elif cur<0 and dict_x_train[_index] not in test_set[0]:
                test_set[0].append(dict_x_train[_index])
                list_count[0] += 1
            flag = True
        ...
        for e in range(len(test_set)):
            if list_count[e] != 20 and dict_x_train[_index] in test_set[e]:
                test_set[e] = test_set[e].replace(dict_x_train[_index], "")
                list_count[e] += 1
                flag = True
        sigma_list.pop(sigma_list.index(cur))
        '''
